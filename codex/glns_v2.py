...
# @Last Modified time: 2024-05-31 15:06:34
import itertools, random, math
from collections import Counter, deque
from codex import note, BigLottery52
...
class random_rb:
    ...
    def get_number_v2(self):
        ...
        dep = sorted(self.nPool[:self.len])
        return dep
            ...
            if self.R != None and self.B != None:
                ...
                match w:
                    ...
                    case 'b':
                        ...
                        rngs_b = [range(1, 17)]
                        conf_r = {
                            "red": partial(BigLottery52.coda_sec, rngs=rngs_r, k=self.rLen),
                        }
                        conf_b = {
                            "blue": partial(BigLottery52.coda_sec, rngs=rngs_b, k=self.bLen),
                        }
                        r = partial(BigLottery52.mark, config=conf_r, rex='value')
                        b = partial(BigLottery52.mark, config=conf_b, rex='value')
                        self.producer.update({'r': r})
                        self.producer.update({'b': b})
                        print('[b] use BigLottery52')
